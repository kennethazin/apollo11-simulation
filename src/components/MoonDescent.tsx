  ...
  useEffect(() => {
    if (cesiumContainerRef.current && !viewerRef.current) {
      ...
      async function initialize() {
        const czmlFilePath = "/lm_descent_trajectory.czml";
        try {
          ...
          const ascentStage = czmlDataSource.entities.getById("AscentStage");
          ...
          if (ascentStage && ascentStage.position) {
            ...
            // Apply a fixed rotation to align the ascent stage model correctly
            const rotationMatrix = Cesium.Matrix3.fromRotationY(
              Cesium.Math.toRadians(90.0)
            );
            const rotationQuaternion =
            ...
            ascentStage.orientation = new Cesium.CallbackProperty(
              (time, result) => {
                const baseOrientation = velocityOrientation.getValue(
                  time,
                  result || new Cesium.Quaternion()
                );
                if (!baseOrientation) return Cesium.Quaternion.IDENTITY;
                return Cesium.Quaternion.multiply(
                  baseOrientation,
                  rotationQuaternion,
                  result || new Cesium.Quaternion()
            ...
            ascentStage.viewFrom = new Cesium.ConstantProperty(
              new Cesium.Cartesian3(-4000, 500, 250) // Adjusted for ascent stage
            );
          }
